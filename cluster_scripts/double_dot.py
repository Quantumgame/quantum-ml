...
N_v = 100 
V_d_vec = np.linspace(50e-3,500e-3,N_v)
output_vec = []
input_vec = []
for i in range(N_v):
    for j in range(N_v):
        b1 = (get_random(-200e-3),get_random(-0.6),get_random(0.05),1)
        d1 = (V_d_vec[i],get_random(-0.2),get_random(0.05),1)
        b2 = (get_random(-250e-3),get_random(0.0),get_random(0.05),1)
        d2 = (V_d_vec[j],get_random(0.2),get_random(0.05),1)
        b3 = (get_random(-200e-3),get_random(0.6),get_random(0.05),1)
        physics_model['K_onsite'] = get_random(50e-3)
        physics_model['list_b'] = [b1,d1,b2,d2,b3]
