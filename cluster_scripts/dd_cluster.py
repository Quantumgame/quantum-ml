...
sys.path.append('/home/ssk4/quantum-ml/nanowire_model')
import physics
...
def calculate_3d_map(ind=0):
    ...
    b2 = [get_random(-250e-3,sigma_mean=0.02),get_random(0.0),get_random(0.05),1]
    d2 = [200e-3,get_random(0.1),get_random(0.05),1]
    b3 = [get_random(-200e-3,sigma_mean=0.02),get_random(0.5),get_random(0.05),1]
    ...
    physics_model['K_onsite'] = np.random.uniform(5e-3,50e-3)
    physics_model['sigma'] = x[1] - x[0]
    ...
    N_v = 100
    V_b1_vec = np.linspace(-50e-3,-300e-3,N_v)
    V_b2_vec = np.linspace(-50e-3,-300e-3,N_v)
    V_b3_vec = np.linspace(-50e-3,-300e-3,N_v)
    output_vec = []
...
def calculate_3d_map(ind=0):
    ...
    import datetime
    dt = str(datetime.datetime.now()) 
    np.save('/wrk/ssk4/dd3d_data/double_dot_3d' + str(N_v) + '_grid_' + dt + '.npy',data)
    # during testing
    #np.save('/Users/ssk4/data/double_dot_3d_test.npy',data)
